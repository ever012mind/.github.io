...
// Algoritmo que calcula o lan챌amento obliquo com resist챗ncia do meio e o efeito magnus.
...
function calculeTypeDistance(distance) {
  ...
  if (distance >= 58.0)
    ...
    return TYPE_DISTANCE.LESS_58; // Esse n찾o precisa j찼 que ele passou do primeiro if, mas deixar assim
...
class Club {
  ...
  // 1W
  rotation_spin = 0.55;      // Rota챌찾o spin
  rotation_curve = 1.61;       // Rota챌찾o curva
...
const _00D66CF8 = 3.0;  // Valor 3 que n찾o sei bem o que 챕
const _00D3D028 = 0.00008; // Efeito Mangnus acho
const _00D1A888 = 12.566371;  // N찾o sei
const _00D3D210 = 25.132742; // N찾o sei
const _00CFF040 = 0.1;
const _00D66CA0 = 0.5;
...
const find_power = (power_player, club_info, shot, power_shot, distancia, altura, vento, angulo, terreno, spin, curva, slope, mira = undefined, percent = undefined) => {
  ...
  do {
    ...
    // 130% 챕 um limite seguro, para ter a % da tacada mesmo que n찾o chega, s처 para saber se estava perto e usar no macro de silvia cannon
    ...
    if (ret == 0)
      ...
      // N찾o tem como achar a for챌a por que nem mandando 100% chega
      // 130% 챕 um limite seguro, para ter a % da tacada mesmo que n찾o chega, s처 para saber se estava perto e usar no macro de silvia cannon
      if (options.percentShot == 1.3 && ret > 0)
        break;

      // N찾o tem como achar a for챌a por que n찾o pode mandar 0.0 de porcentagem
...
function calc(el) {
  ...
  if (f[index_f].power != -1) {
    ...
    result.color = 'Green';
    result.innerHTML = `
	<text style="color:gray"><text style="font-size:20px">
		Power : ${(f[index_f].power * 100).toFixed(2)}% /
	</text>	<text style="color:Gray"><text style="font-size:20px">
		이전 캘리  = ${(Math.floor(f[index_f].power / (1 / 360)) * (1 / 360) * f[index_f].power_range).toFixed(1)}y <br>
	</text> 

	<text style="color:gray"><text style="font-size:20px">
		장판 : <strong>${(((desvioByDegree(f[index_f].desvio, distance) / 0.21656) * dis)*0.8476487854 / aim).toFixed(4)}</strong><br>
<text style="color:green"><text style="font-size:35px"><strong>스마트 : ${(((desvioByDegree(f[index_f].desvio, distance) / 0.21656) * dis)*0.8476487854*0.25*(10.578) / aim).toFixed(3)} </strong>
	</text><br>

	<text style="color:red"><text style="font-size:35px">
		<strong>샷 힘 : ${(Math.ceil(f[index_f].power / (1 / 360)) * (1 / 360) * f[index_f].power_range).toFixed(1)}y </strong>  	<text style="color:purple"><text style="font-size:20px">
		(실제 힘: ${(f[index_f].power_range * f[index_f].power).toFixed(1)} y)
	</text><br>

<text style="color:gray"><text style="font-size:16px">
		(스마트 26.445칸이 장판 10.0칸 / 화면끝은 좌우로 26.69칸)

		</text><br>`;

  } else {
    result.color = 'DarkRed'
    result.innerHTML = '결과값을 찾을 수 없습니다.'
  }
}
